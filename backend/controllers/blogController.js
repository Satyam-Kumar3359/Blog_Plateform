...
export const blogPost = catchAsyncErrors(async (req, res, next) => {
  ...
  if (
    ...
    (paraThreeImage && !allowedFormats.includes(paraThreeImage.mimetype))//mimetype (extenstion like .png,.jpeg)is used to check the file type
  ) {
    return next(
      new ErrorHandler(
...
// It first checks if the blog exists, and if it does, it deletes it from the
...
// It first checks if the blog exists, and if it does, it updates its details
export const updateBlog = catchAsyncErrors(async (req, res, next) => {
  ...
  if (req.files) {
    ...
    if (req.files && mainImage) {// Check if mainImage exists in the request files 
 ...
 // Check if paraOneImage, paraTwoImage, and paraThreeImage exist in the request files or not if not then why it will not delete the image
    if (req.files && paraOneImage) {
      ...
      //if paraOneImage exists, then it will be destroy and upload new image to cloudinary
